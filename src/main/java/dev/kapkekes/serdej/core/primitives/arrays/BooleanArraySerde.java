...
import dev.kapkekes.serdej.core.primitives.BooleanSerde;

import java.io.IOException;
import java.util.ArrayList;

@RegisterSerde
public class BooleanArraySerde implements PrimitiveSerde {
    private final BooleanSerde serde = new BooleanSerde();
    ...
    public boolean[] deserialize(JsonParser parser) throws IOException {
        ...
        if (!parser.isExpectedStartArrayToken()) {
            throw new JsonParseException(parser, String.format("First token expected to be JsonToken.START_ARRAY for boolean[] (JsonToken.%s provided)", parser.currentToken()), parser.currentLocation());
        }

        final var arrayList = new ArrayList<Boolean>();
        ...
        final var array = new boolean[arrayList.size()];
