...
import dev.kapkekes.serdej.core.primitives.FloatSerde;

import java.io.IOException;
import java.util.ArrayList;

@RegisterSerde
public class FloatArraySerde implements PrimitiveSerde {
    private final FloatSerde serde = new FloatSerde();
    ...
    public float[] deserialize(JsonParser parser) throws IOException {
        ...
        if (!parser.isExpectedStartArrayToken()) {
            throw new JsonParseException(parser, String.format("First token expected to be JsonToken.START_ARRAY for float[] (JsonToken.%s provided)", parser.currentToken()), parser.currentLocation());
        }

        final var arrayList = new ArrayList<Float>();
        ...
        final var array = new float[arrayList.size()];
