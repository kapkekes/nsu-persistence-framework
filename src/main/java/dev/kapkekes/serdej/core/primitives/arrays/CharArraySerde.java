...
import dev.kapkekes.serdej.core.primitives.CharSerde;

import java.io.IOException;
import java.util.ArrayList;

@RegisterSerde
public class CharArraySerde implements PrimitiveSerde {
    private final CharSerde serde = new CharSerde();
    ...
    public char[] deserialize(JsonParser parser) throws IOException {
        ...
        if (!parser.isExpectedStartArrayToken()) {
            throw new JsonParseException(parser, String.format("First token expected to be JsonToken.START_ARRAY for char[] (JsonToken.%s provided)", parser.currentToken()), parser.currentLocation());
        }

        final var arrayList = new ArrayList<Character>();
        ...
        final var array = new char[arrayList.size()];
